...
async function cleanup({ runId } = {}) {
  ...
  await db.sequelize.transaction(async (transaction) => {
    ...
    if (orderIds.length) {
      ...
      counts.reservations = await db.InventoryReservation.destroy({ where: { order_id: { [Op.in]: orderIds } }, transaction });
...
async function verify() {
  ...
  return {
    ...
    reservations_in_e2e_db: await db.InventoryReservation.count(),
